...
export default async function CategoryPage({
  ...
  if (!category || (!category.subcategories && !category.groups)) notFound();
    ...
    <div>
      ...
      <div className="px-5 pb-8">
        ...
        <div className="flex flex-col gap-8">
          {groups.map((group) => {
            const items = getItemsByGroup(category.id, group.id);
            if (items.length === 0) return null;
            return (
              <section key={group.id}>
                <h2 className="mb-3 font-display text-lg text-gold-soft">
                  {group.name}
                </h2>
